package page;

public class CaramelWriterProperties {
	public static String TAG_P_START= "<p style=\"font-size:12pt;\">";
	public static String TAG_P_START_TITLE= "<p style=\"font-size:18pt; color: rgb(255, 108, 0);\">";
	public static String TAG_P_END= "</p>";
	
	public static String MARGIN = "<br /><br /><br />";
	public static String MARGIN_1 = "<br />";
	public static String MARGIN_2 = "<br /><br />";
	
	
	public static String CLASSIFICATION_HOSPITAL = "병원(성)";
	public static String CLASSIFICATION_NORMAL = "일반";
	public static String CLASSIFICATION_RESTAURANT = "식당";
	public static String CLASSIFICATION_HOSPITAL2 = "병원(일)";
	public static String CLASSIFICATION_KP = "카프";
	public static String CLASSIFICATION_STUDY = "학원";
	
	
	public static int SUBJECT_COUNT = 5; 
	
	public static String PHRASES = "<p><span style=\"font-size: 18pt; background-color: rgb(0, 0, 0); color: rgb(255, 255, 255);\">[이 포스팅은 소정의 원고료를 제공받아 작성되었습니다.]</span></p>";
}
